...
For GALBA, the protein database contained Metazoa proteins from OrthoDB v12 supplemented with available ascidian proteins. Duplicate proteins and sequences shorter than 50 amino acids were removed. The final file is `metazoa_plus_ascidiacea.rmdup.min50.fa`.

For GeMoMa, published genome assemblies and transcriptome-supported annotations (GFF) were retrieved for representatives of all three ascidian orders from NCBI and Tunome:

| Order           | Reference species        | Genome/GFF prefix | GeMoMa identifier |
| --------------- | ------------------------ | ----------------- | ----------------- |
| Aplousobranchia | *Aplidium turbinatum*    | `Atur`            | `bput`            |
| Aplousobranchia | *Clavelina lepadiformis* | `Clep`            | `bris`            |
| Aplousobranchia | *Diplosoma virens*       | `Dvir`            | `isca`            |
| Phlebobranchia  | *Ascidiella aspersa*     | `Aasp`            | `asim`            |
| Phlebobranchia  | *Ascidia mentula*        | `Amen`            | `bras`            |
| Phlebobranchia  | *Phallusia fumigata*     | `Pfum`            | `dmel`            |
| Stolidobranchia | *Styela clava*           | `Scla`            | `iele`            |
| Stolidobranchia | *Polycarpa mamillaris*   | `Pmam`            | `rmai`            |
| Stolidobranchia | *Botrylloides leachii*   | `Bleachii`        | `ztil`            |
...
apptainer exec \
  ...
  --prot_seq=metazoa_plus_ascidiacea.rmdup.min50.fa \ #this file is saved in: /nesi/project/uow04282/uniprot_and_orthodb12
  --gff3 \
  --AUGUSTUS_ab_initio \
  --threads=24
```
